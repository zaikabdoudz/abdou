const handler = async (m, { conn, args }) => {
  try {
    ...
    if (groupMetadata.announce === false) return conn.reply(m.chat, `《✧》 المجموعة مفتوحة بالفعل.`, m)
    const applyAction = async () => {
      await conn.groupSettingUpdate(m.chat, 'not_announcement')
      return conn.reply(m.chat, `✿ تم فتح المجموعة بنجاح.`, m)
    }
    if (timeout > 0) {
      await conn.reply(m.chat, `❀ ستفتح المجموعة بعد ${clockString(timeout)}.`, m)
      setTimeout(async () => {
        try { const md = await conn.groupMetadata(m.chat); if (md.announce === false) return; await applyAction() } catch {}
      }, timeout)
    } else {
      await applyAction()
    }
...
handler.help = ['فتح', 'فتح 10m']
handler.tags = ['group']
handler.command = /^(فتح|open)$/i
handler.admin = true

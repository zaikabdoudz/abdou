...
// BailMod / Baileys يمكن أن يصدّر بطرق مختلفة
// نحاول الحصول على الـ exports الصحيحة
let _baileysExports
if (_baileys.proto) {
    // صادر مباشرة بدون default
    _baileysExports = _baileys
} else if (_baileys.default?.proto) {
    // صادر عبر default
    _baileysExports = _baileys.default
} else if (typeof _baileys.default === 'function') {
    // الـ default هو الـ makeWASocket مباشرة
    _baileysExports = _baileys
} else {
    _baileysExports = _baileys.default || _baileys
}

// Debug: نعرض المفاتيح المتاحة
console.log('[BailMod] Available exports:', Object.keys(_baileysExports).slice(0, 20))

const {
    makeWASocket: _makeWaSocketNamed,
    default: _makeWaSocketDefault,
    makeWALegacySocket,
    proto,
...
console.log('[BailMod] proto:', typeof proto, '| proto.WebMessageInfo:', typeof proto?.WebMessageInfo)
console.log('[BailMod] makeWASocket:', typeof _makeWaSocketNamed, '| default:', typeof _makeWaSocketDefault)

// الـ makeWASocket الصحيح حسب طريقة التصدير
const _makeWaSocket = _makeWaSocketNamed || _makeWaSocketDefault || _baileysExports.default
...
export function smsg(conn, m, hasParent) {
    ...
    if (!proto?.WebMessageInfo) {
        console.error("[smsg] proto.WebMessageInfo غير متاح");
        return m;
    }
...
export function serialize() {
    ...
    if (!proto?.WebMessageInfo?.prototype) {
        console.error("[serialize] proto.WebMessageInfo غير متاح، تخطي serialize()");
        return;
    }
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                try {
                    const raw = this.key?.fromMe && this.conn?.user.id || this.participant || this.key?.participant || this.chat || ''
                    const decoded = this.conn?.decodeJid(raw) || raw
                    // ✅ إصلاح: لو @lid نحاول نلاقي phoneNumber من participants
                    if (decoded && decoded.endsWith('@lid')) {
                        const lidNum = decoded.split('@')[0]
                        // نبحث في chats cache عن phoneNumber المقابل
                        const chatId = this.chat
                        if (chatId && this.conn?.chats?.[chatId]?.metadata?.participants) {
                            const found = this.conn.chats[chatId].metadata.participants.find(
                                p => (p.id || '').split('@')[0] === lidNum
                            )
                            if (found?.phoneNumber) return found.phoneNumber
                        }
                        // fallback: نرجع @lid كما هو - handler.js يتعامل معه
                        return decoded
                    }
                    return decoded
                } catch {
        ...
        mtype: {
            get() {
                try {
                    ...
                    // viewOnceMessage اللي يحتوي على interactiveResponseMessage
                    const viewOnce = message?.viewOnceMessage?.message || message?.viewOnceMessageV2?.message
                    if (viewOnce?.interactiveResponseMessage) return "interactiveResponseMessage";
                                ...
                                get() {
                                    try {
                                        ...
                                        if (parsedJid && parsedJid.includes("@lid")) {
                                            ...
                                            if (groupChatId) {
                                                // ✅ إصلاح: بحث في cache بدل resolveLidToRealJid
                                                const lidNum = parsedJid.split("@")[0]
                                                const cachedMeta = self.conn?.chats?.[groupChatId]?.metadata
                                                if (cachedMeta?.participants) {
                                                    const found = cachedMeta.participants.find(
                                                        p => (p.id || "").split("@")[0] === lidNum
                                                    )
                                                    if (found?.phoneNumber) return found.phoneNumber
                                                }
        ...
        text: {
            get() {
                try {
                    ...
                    // ✅ استخراج نص الزر المضغوط (interactiveResponseMessage)
                    if (message?.interactiveResponseMessage) {
                        const resp = message.interactiveResponseMessage
                        const body = resp?.nativeFlowResponseMessage?.paramsJson
                        if (body) {
                            try {
                    ...
                    // ✅ viewOnceMessage اللي يحتوي على interactiveResponseMessage
                    const viewOnce = message?.viewOnceMessage?.message || message?.viewOnceMessageV2?.message || message?.viewOnceMessageV2Extension?.message
                    if (viewOnce?.interactiveResponseMessage) {
                        const resp = viewOnce.interactiveResponseMessage
                        const body = resp?.nativeFlowResponseMessage?.paramsJson
                        if (body) {
                            try {
                                const parsed = JSON.parse(body)
                    ...
                    if (message?.buttonsResponseMessage) {
                        return message.buttonsResponseMessage.selectedButtonId || message.buttonsResponseMessage.selectedDisplayText || ""
                    }

                    // ✅ listResponseMessage
                    if (message?.listResponseMessage) {
                        return message.listResponseMessage.singleSelectReply?.selectedRowId || message.listResponseMessage.title || ""
                    }
        ...
        mentionedJid: {
            get() {
                try {
                    ...
                    // ✅ إصلاح: يرجع array مباشرة (synchronous) مو Promise
                    // لأن جميع الـ plugins تستخدم m.mentionedJid[0] بدون await
                    return mentioned.map(user => {
                        try {
                            ...
                            if (typeof user === "string" && user.includes("@lid") && groupChatId) {
                                // نبحث عن phoneNumber في cache
                                const lidNum = user.split("@")[0]
                                const cachedMeta = this.conn?.chats?.[groupChatId]?.metadata
                                if (cachedMeta?.participants) {
                                    const found = cachedMeta.participants.find(
                                        p => (p.id || "").split("@")[0] === lidNum
                                    )
                                ...
                                // fallback: نرجع @lid - promote/kick يتعاملون معه
                                return user
                            }
                            return user || "";
                        } catch (e) {
